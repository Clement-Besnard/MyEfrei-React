      ...
      data.eleves.forEach(eleve => {
        if (eleve.notes && eleve.notes.length > 0) {
          eleve.notes.forEach(note => {
            const evaluationKey = `${note.description}_${note.date}`;
            
            if (!evaluationsMap[evaluationKey]) {
              evaluationsMap[evaluationKey] = {
                id: evaluationKey,
            ...
            evaluationsMap[evaluationKey].notes[eleve.id] = {
              id: note.id,
              note: note.note,
              eleveId: eleve.id,
  ...
  const saveNotes = async () => {
    ...
    try {
      for (const noteData of notesToSave) {
        const existingNote = evaluation.notes[noteData.eleveId];
        
        if (existingNote) {
          await fetch(`${process.env.REACT_APP_BACKEND_URL}/prof/notes/${existingNote.id}`, {
            method: 'PATCH',
            headers: {
      ...
      await fetchNotesByMatiere(selectedMatiereId);
    ...
    try {
      ...
      const deletePromises = noteIds.map(noteId => 
        fetch(`${process.env.REACT_APP_BACKEND_URL}/prof/notes/${noteId}`, {
          ...
          body: JSON.stringify({ professorId: auth.user.id }),
        })
      );
